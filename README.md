...
発電機起動停止計画（ユニット・コミットメント）の例題です。単一エリア版（UC）と 3 エリア連系版（SCUC）の 2 種類を実装しています。
...
| `main_scuc.py` | **3 エリア SCUC**：エリア間連系線付き。需給・太陽光・風力はエリア別。求解後に **HTML レポート**（手法・条件・結果・考察のタブ）を `results/` に出力。 |
...
- **レポート内容**:
  ...
  - **Consideration**: 結果サマリ表（コスト・連系線利用率・太陽光抑制率など）とまとめ

ブラウザで該当 HTML を開くとタブ切り替えで閲覧できます。

## モデル概要（SCUC）

- **3 エリア**: Area1（G1–G10）, Area2（G11–G20）, Area3（G21–G30）。各エリアで需要・太陽光・風力が定義され、需給バランスを満たしつつ連系線で融通。
- **連系線**: 直列トポロジー（Area1—Area2—Area3）。潮流は ±F_max 以内。
