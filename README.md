...
| ファイル | 説明 |
|----------|------|
| `main.py` | 単一エリア UC：需要・太陽光・風力・火力。需給バランス・予備力・MUT/MDT・ランプ制約。結果は PNG で `results/` に出力。 |
...
- **長期解析について**: 解析対象日数は **最大 365 日**まで入力できますが、MILP の規模が非常に大きくなるため計算時間・メモリ消費が急増します。レポート生成の負荷を抑えるため、**結果図（Results）は先頭 7 日（168 時間）のみ可視化**します（最適化自体は指定期間で実行）。
- **レポート内容**:
...
- **5 エリア**: Area1〜Area5。各エリアで需要・太陽光・風力が定義され、需給バランスを満たしつつ連系線で融通。発電機は **各エリア 20 機（合計 100 機）**。
